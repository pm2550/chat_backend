...
public class MessageController {
    /** 加密缩略图的上限：客户端做的是长边 720px 的小图（老版本 400px），正常几十 KB。 */
    private static final long MAX_ENCRYPTED_THUMBNAIL_BYTES = 512 * 1024;
    /** 加密中图的上限：客户端只给大原图做长边 1280px 的中图，正常一两百 KB。 */
    private static final long MAX_ENCRYPTED_PREVIEW_BYTES = 2 * 1024 * 1024;
    ...
    public ResponseEntity<?> sendFileMessage(
        ...
        try {
            ...
            // 预览图（缩略图 + 大图的中图）：明文图片由服务器生成；
            // 加密附件服务器看不到图，用客户端加密好一并传来的那两张。
            ImageThumbnailService.MessageRenditions renditions = ImageThumbnailService.MessageRenditions.NONE;
            if (encryptedAttachment) {
                renditions = ImageThumbnailService.MessageRenditions.clientSealed(
                        storeEncryptedRendition(thumbnail, MAX_ENCRYPTED_THUMBNAIL_BYTES, "缩略图"),
                        storeEncryptedRendition(preview, MAX_ENCRYPTED_PREVIEW_BYTES, "中图"));
            } else if (messageType == Message.MessageType.IMAGE) {
                renditions = ImageThumbnailService.MessageRenditions.serverGenerated(
                        imageThumbnailService.createAndStore(file.getBytes()));
            }
    ...
    public ResponseEntity<?> sendFileMessage(
        ...
        return sendFileMessage(chatRoomId, file, requestedMessageType, encryptedContent,
                encryptionVersion, clientMessageId, null, null, auth);
    }

    /**
     * 加密附件的缩略图/中图是客户端各用一把随机密钥加密的密文（密钥在消息信封里），
     * 和附件本身一样存成 .bin；超过上限就不要了（客户端会退回缩略图或原图），不拒绝整条消息。
     */
    private String storeEncryptedRendition(MultipartFile part, long maxBytes, String label)
            throws java.io.IOException {
        if (part == null || part.isEmpty()) {
            return null;
        }
        if (part.getSize() > maxBytes) {
            log.warn("加密{}过大，忽略: {} bytes", label, part.getSize());
            return null;
        }
        return fileStorageService.uploadEncryptedChatFile(part);
    }
    ...
    public ResponseEntity<?> sendFileFromUrl(
        ...
        try {
            ...
            String fileUrl = fileStorageService.uploadChatImageBytes(
                    ...
                    image.contentType(),
                    image.bytes());
            var renditions = ImageThumbnailService.MessageRenditions.serverGenerated(
                    imageThumbnailService.createAndStore(image.bytes()));
